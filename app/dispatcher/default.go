...
func (d *DefaultDispatcher) getLink(ctx context.Context) (*transport.Link, *transport.Link, error) {
	...
	if user != nil && len(user.Email) > 0 {
		...
		bucket, ok, reject := d.Limiter.GetLimiter(
			...
			sessionInbound.Source.Address.String(),
		)
		...
		if reject {
			...
			common.Interrupt(outboundLink.Reader)
			common.Interrupt(inboundLink.Reader)
			return nil, nil, newError("Subscription IP Limit Exceeded for: ", user.Email)
		}
...
func (d *DefaultDispatcher) WrapLink(ctx context.Context, policyManager policy.Manager, statsManager stats.Manager, link *transport.Link) (*transport.Link, error) {
	sessionInbound := session.InboundFromContext(ctx)
	var user *protocol.MemoryUser
	if sessionInbound != nil {
		sessionInbound.CanSpliceCopy = 3
	...
	if user != nil && len(user.Email) > 0 {
		...
		if ok {
			...
			link.Reader = d.Limiter.RateTimeoutReader(link.Reader.(*buf.TimeoutWrapperReader), bucket)
		}
		...
		if p.Stats.UserUplink {
			...
			if c, _ := stats.GetOrRegisterCounter(statsManager, name); c != nil {
				// Check if reader is wrapped with rate limiter
				if tr, ok := link.Reader.(*limiter.TimeoutReader); ok {
					// Access the inner TimeoutWrapperReader through the rate limiter wrapper
					tr.Reader.(*buf.TimeoutWrapperReader).Counter = c
				} else if tr, ok := link.Reader.(*buf.TimeoutWrapperReader); ok {
					// If no rate limiter, access directly
					tr.Counter = c
				}
			}
		}
...
func (d *DefaultDispatcher) DispatchLink(ctx context.Context, destination net.Destination, outbound *transport.Link) error {
	...
	var errr error
	outbound, errr = d.WrapLink(ctx, d.policy, d.stats, outbound)
	if errr != nil {
		return errr
	}
	sniffingRequest := content.SniffingRequest
